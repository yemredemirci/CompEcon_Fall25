...
TeX is a typesetting system.  It's quite helpful for creating scientific documents as it makes the process of writing equations and compiling your bibliography much easier.
...
* Windows: [proTeXt](https://www.tug.org/protext/) (comes with [TeXStudio](http://texstudio.sourceforge.net) frontend)
...
[VS Code](https://code.visualstudio.com) is free and is a very capable text editor. It includes syntax highlighting for Python and and built-in Git controls.  In addition to the basics, you may want to use a more advanced linter for Python.  This will help you correct syntax errors on the fly and provide helpful information as you declare objects and call functions.  [This link](https://code.visualstudio.com/docs/python/linting) provides step-by-step instructions on using more advanced linting in VS Code.  The [Github Copilot](https://github.com/features/copilot) extension is also very valuable and highly recommended.

Other extensions that I recommend installing into your VS Code:
* cornflakes-linter
* Git Extension Pack
* GitLens
* Jupyter
...
I recommend that you download the Anaconda distribution of Python provided by [Anaconda](https://anaconda.org). I recommend the most recent stable version of Python, which is currently Python 3.13. This can be done from the [Anaconda download page](https://www.anaconda.com/downloads) for Windows, Mac OSX, and Linux machines. The code we will be writing uses common Python libraries such as `NumPy`, `SciPy`, `pickle`, `os`, `matplotlib`, and `time`.  **For Windows users:** when installing, please click to add the Anaconda distribution to your PATH environment variable (this is not the default option).
...
In the text editors Atom and Sublime Text 3 you can install Linter packages that highlight areas of your code that break PEP 8 rules and tell you what the violation is.  I also recommend using a package to automatically format code, such as [`black`](https://pypi.org/project/black/)


## 7. Jupyter Notebooks

[Jupyter notebooks](http://jupyter.org/) are files that end with the `*.ipynb` suffix. These notebooks are opened in a browser environment and are an open source web application that combines instructional text with live executable and modifyable code for many different programming platforms (e.g., Python, R, Julia). Jupyter notebooks are an ideal tool for teaching programming as they provide the code for a user to execute and they also provide the context and explanation for the code. There are some Jupyter notebooks you can use as examplies in the [Optimizatio materials](https://github.com/OpenRG/US-ITC/tree/main/Optimization) folder of this repository.
